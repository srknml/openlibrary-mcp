...
import {
  ...
  getAuthorWorks,
  getBooksBySubject,
} from "./api/openLibrary.js";
import type { OLAuthorDetail, OLBookData } from "./types/openLibrary.js";
 ...
 */
function extractBio(bio: OLAuthorDetail["bio"]): string {
 ...
 */
function formatList(items: string[], header?: string): string {
...
server.registerTool(
  ...
  {
    description:
      "Search for books in the Open Library catalog. Returns titles, authors, publication years, ISBNs, and subjects. " +
  ...
  async ({ query, limit, offset, author, subject, language }) => {
    ...
    if (!data) {
      return errorResult(
        "Failed to reach the Open Library search API. The service may be temporarily unavailable."
      );
    }

    if (data.docs.length === 0) {
      return textResult(
        `No books found for query: "${query}"${author ? ` by "${author}"` : ""}. Try a broader search.`
      );
    }
    ...
    const header =
      `Search: "${query}" | Found ${data.numFound.toLocaleString()} total | ` +
...
server.registerTool(
  ...
  {
    description:
      "Get detailed information about a specific book using its ISBN-10 or ISBN-13. " +
    ...
    inputSchema: {
      isbn: z
        ...
        .max(17)
        .describe(
          "ISBN-10 or ISBN-13 with or without hyphens, e.g. '9780441172719' or '978-0-441-17271-9'"
        ),
    },
  },
  async ({ isbn }) => {
    ...
    const book: OLBookData | undefined = data[bibkey];
...
server.registerTool(
  ...
  {
    description:
      "Search for authors by name in the Open Library catalog. " +
...
server.registerTool(
  ...
  {
    description:
      "Get detailed information about an author using their Open Library author ID (OL ID). " +
    ...
    inputSchema: {
      ol_author_id: z
        ...
        .min(1)
        .describe(
          "Open Library author ID, e.g. 'OL26320A'. Can include or omit the '/authors/' prefix."
        ),
    },
  },
  async ({ ol_author_id }) => {
    ...
    if (!data) {
      return errorResult(
        `Failed to fetch author details for ID: "${ol_author_id}". ` +
...
server.registerTool(
  ...
  {
    description:
      "Get a list of works (books) by an author using their Open Library author ID. " +
  ...
  async ({ ol_author_id, limit }) => {
    ...
    const header =
      `Author ID: ${ol_author_id} | Total works: ${data.size} | Showing ${data.entries.length}`;
...
server.registerTool(
  ...
  {
    description:
      "Browse books by subject or topic using the Open Library subject catalog. " +
    ...
    inputSchema: {
      subject: z
        ...
        .min(1)
        .describe(
          "Subject or topic to browse. Spaces become underscores automatically, e.g. 'science fiction' → 'science_fiction'"
        ),
      limit: z
        .number()
        .int()
  ...
  async ({ subject, limit }) => {
    ...
    if (!data) {
      return errorResult(
        `Failed to fetch books for subject: "${subject}". The API may be temporarily unavailable.`
      );
    }

    if (!data.works || data.works.length === 0) {
      return textResult(
        `No books found for subject: "${subject}". ` +
...
server.registerResource(
  "book-by-isbn",
  new ResourceTemplate("openlibrary://book/{isbn}", { list: undefined }),
  {
    title: "Open Library Book",
    description:
      "Full book data from Open Library, fetched by ISBN. " +
...
server.registerResource(
  "author-by-id",
  new ResourceTemplate("openlibrary://author/{ol_id}", { list: undefined }),
  {
    title: "Open Library Author",
    description:
      "Author data from Open Library, fetched by Open Library author ID. " +
