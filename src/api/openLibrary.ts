...
import type {
  OLSearchResponse,
  OLAuthorSearchResponse,
  OLAuthorDetail,
  OLAuthorWorksResponse,
  OLBookByISBNResponse,
  OLSubjectResponse,
...
async function fetchJSON<T>(url: string): Promise<T | null> {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `[OpenLibrary] HTTP ${response.status} ${response.statusText} — GET ${url}`
      );
      return null;
    }
...
export async function searchBooks(
  ...
  return fetchJSON<OLSearchResponse>(`${BASE_URL}/search.json?${params.toString()}`);
}
 ...
 */
export async function getBookByISBN(isbn: string): Promise<OLBookByISBNResponse | null> {
  ...
  return fetchJSON<OLBookByISBNResponse>(`${BASE_URL}/api/books?${params.toString()}`);
}
...
export async function searchAuthors(
  query: string,
  limit = 10,
): Promise<OLAuthorSearchResponse | null> {
  const params = new URLSearchParams({
    q: query,
    limit: String(limit),
  });

  return fetchJSON<OLAuthorSearchResponse>(
...
export async function getAuthorDetails(
  ...
  return fetchJSON<OLAuthorDetail>(`${BASE_URL}/authors/${id}.json`);
}
...
export async function getAuthorWorks(
  olAuthorId: string,
  limit = 20,
): Promise<OLAuthorWorksResponse | null> {
  const id = olAuthorId.replace(/^\/authors\//, "");
  const params = new URLSearchParams({ limit: String(limit) });

  return fetchJSON<OLAuthorWorksResponse>(
    `${BASE_URL}/authors/${id}/works.json?${params.toString()}`
  );
}
...
export async function getBooksBySubject(
  subject: string,
  limit = 20,
): Promise<OLSubjectResponse | null> {
  ...
  return fetchJSON<OLSubjectResponse>(
    `${BASE_URL}/subjects/${normalized}.json?${params.toString()}`
  );
}
